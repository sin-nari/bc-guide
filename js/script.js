/**
 * 부천형 고등학교 교육과정·학생평가 길라잡이 — 메인 스크립트 v3.0
 * ─────────────────────────────────────────────────────────────────
 *  1. 초기화
 *  2. 네비게이션 / 사이드바 (준비중 메뉴 완전 숨김)
 *  3. 홈 (오늘의 바로가기 배너, 빠른시작 4개)
 *  4. 교육과정
 *  5. 학생평가
 *  6. 질의회신 DB (카드 + 모달, 답변전문, 출처유형)
 *  7. 자료 아카이브
 *  8. STORY
 *  9. 통합 검색 (그룹별 결과)
 * 10. FAQ (답변전문 포함)
...
/* ══════════════════════════════════════════════════════════════
   2. 네비게이션 / 사이드바
      ★ 준비중(ready:false) 메뉴는 사이드바에서 완전히 숨깁니다.
   ══════════════════════════════════════════════════════════════ */
...
/* ── 사이드바 설정 ── ready:false 항목은 아예 표시 안 함 ──── */
const SIDEBAR_CONFIG = {
  ...
  curriculum: {
    ...
    items:[
      ...
      {
        id:'consulting', label:'2027 교육과정 편성 컨설팅', icon:'🗂️', ready:true,
        children:[
      ...
      /* 아래 항목들은 ready:false → 사이드바에 표시 안 됨 */
  ...
  story: {
    ...
    items:[
      ...
      {id:'expo-story',     label:'미래공감 설명회',    icon:'🎤', ready:true},
...
function renderSidebar(page, subMenu) {
  ...
  /* ★ ready:false 항목은 완전히 건너뜀 */
  cfg.items.filter(item => item.ready).forEach(item => {
    const isActive  = subMenu === item.id;
    const hlClass   = item.highlight ? ' hl-item' : '';
    const hlBadge   = item.highlight ? '<span class="hl-badge">★</span>' : '';
    const extBadge  = item.extLink   ? '<span class="sidebar-soon" style="color:rgba(255,255,255,.45)">↗</span>' : '';

    if (item.children?.length) {
      const childActive = item.children.some(c => c.id === subMenu);
      html += `
        <div class="sidebar-nav-item sidebar-parent${isActive||childActive?' active':''}${hlClass}"
        ...
        <div class="sidebar-submenu${childActive?' open':''}" id="sub-${item.id}">
          ${item.children.map(c => `
            <div class="sidebar-nav-item${subMenu===c.id?' active':''}"
      ...
      html += `
        <div class="sidebar-nav-item${isActive?' active':''}${hlClass}"
             data-page="${page}" data-submenu="${item.id}">
          <span class="item-icon">${item.icon}</span>
          <span class="item-label">${item.label}</span>${hlBadge}${extBadge}
        </div>`;
    }
  });
...
function renderCurriculumPage(subMenu) {
  ...
  if (subMenu.startsWith('planning')) {
    ...
    const m = {
      'planning-notice':'notice','planning-check':'checklist',
  ...
  if (subMenu.startsWith('consulting')) {
    ...
    const m = {
      'consulting-schedule':'schedule','consulting-checkpoint':'checkpoint',
      'consulting-refs':'refs','consulting-faq':'faq'
    };
    activateTab(m[subMenu]||'overview');
    initFAQ(); return;
  }
  if (subMenu==='joint'||subMenu==='seasonal') {
    c.innerHTML = renderExtLinkPage(subMenu); return;
  }
  c.innerHTML = renderCurriculumHome();
}
...
function renderCurriculumHome() {
  ...
  <div style="padding:1.8rem 2.5rem">
    <div class="alert alert-info">
      ...
      <span><strong>교육과정 편성·운영</strong>과 <strong>2027 교육과정 편성 컨설팅</strong>이 우선 활성화되어 있습니다.</span>
    </div>
    <div class="section-header"><span class="section-title">⚡ 활성화 메뉴</span></div>
    <div class="quick-grid" style="margin-bottom:1.8rem">
      ${[
        {icon:'📋',title:'교육과정 편성·운영',     sub:'planning',  color:'blue',  desc:'편성 절차·유의사항·체크리스트'},
        {icon:'🗂️',title:'2027 교육과정 편성 컨설팅',sub:'consulting',color:'indigo',desc:'일정·대상·컨설팅 체크포인트'},
      ].map(c=>`
    ...
    <div class="section-header"><span class="section-title">🔗 외부 길라잡이 연결</span></div>
...
function renderConsultingPage() {
  ...
  return `
  <div class="page-header">
    <div class="page-header-breadcrumb">🏠 홈 <span>›</span> 고등학교 교육과정 <span>›</span> 2027 교육과정 편성 컨설팅</div>
    <div class="page-header-title">🗂️ 2027 교육과정 편성 컨설팅</div>
...
function renderExtLinkPage(subMenu) {
  ...
  <div style="padding:1.8rem 2.5rem">
    <div class="alert alert-info">
      <span class="alert-icon">ℹ️</span>
      <span>${label} 길라잡이는 별도 사이트에서 운영됩니다.</span>
    </div>
    ${ext?`
    <div class="ext-link-card" style="max-width:380px">
...
function renderEvaluationPage(subMenu) {
  ...
  <div class="coming-soon-section">
    ...
    <div class="coming-soon-desc">아래 우선 활성화 메뉴를 이용해 보세요.</div>
...
function renderNeisPage() {
  ...
  <div style="padding:1.8rem 2.5rem">
    ...
    <div class="section-header" style="margin-top:.5rem"><span class="section-title">📂 관련 서식</span></div>
    <div class="resource-grid">
...
function renderQnaPage(subMenu) {
  ...
  <div class="page-header">
    ...
    <div class="page-header-desc">근거 기반 교육과정·학생평가 지식 DB — 답변전문 + 출처유형 포함</div>
    <div style="height:1.1rem"></div>
  </div>
  <div style="padding:1.8rem 2.5rem">
    ...
    <div class="faq-filters" id="qna-filters">
      <button class="filter-chip${!activeCat?' active':''}" onclick="filterQnaCards('')">
        전체 (${SITE_DATA.qnaCases.length})
      </button>
      ${['학생평가','교육과정','나이스'].map(cat=>{
        const cnt=SITE_DATA.qnaCases.filter(q=>q.대분류===cat).length;
        return `<button class="filter-chip${activeCat===cat?' active':''}"
                        onclick="filterQnaCards('${cat}')">${cat} (${cnt})</button>`;
      }).join('')}
    </div>
...
function filterQnaCards(cat) {
  ...
  if (cat!==undefined) {
    document.querySelectorAll('#qna-filters .filter-chip').forEach(c=>{
      c.classList.toggle('active', c.textContent.trim().startsWith(cat||'전체'));
    });
  }

  const chips = document.querySelectorAll('#qna-filters .filter-chip.active');
  const activeCat = chips.length
    ? chips[0].textContent.trim().replace(/\s*\(\d+\)/,'') === '전체' ? '' : chips[0].textContent.trim().replace(/\s*\(\d+\)/,'')
    : '';
  ...
  grid.querySelectorAll('.qna-card').forEach(card=>{
    ...
    card.style.display=(matchCat&&matchSource&&matchQ)?'':'none';
  });
}
...
function renderArchivePage() {
  ...
  <div class="page-header">
    ...
    <div class="page-header-desc">실무 활용도 중심 자료 모음</div>
    <div style="height:1.1rem"></div>
  </div>
  <div style="padding:1.8rem 2.5rem">
    <div class="access-legend">
...
function renderResourceCard(r) {
  ...
  const ev      = r.근거문서명?`${r.근거문서명}${r.해당페이지?' '+r.해당페이지:''}`:'';
  return `
...
function searchAll(query) {
  ...
  SITE_DATA.qnaCases.forEach(qc=>{
    if((qc.질문||'').toLowerCase().includes(q)||(qc.답변요약||'').toLowerCase().includes(q)||(qc.태그||[]).join(' ').toLowerCase().includes(q))
      groups['📚 질의회신'].push({title:qc.질문||'',meta:`${qc.대분류} · ${qc.출처유형||''}`,action:()=>{navigateTo('qna');setTimeout(()=>openQnaModal(qc.ID),300);}});
  });
  SITE_DATA.faqs.forEach(f=>{
    if((f.질문||'').toLowerCase().includes(q)||(f.답변요약||'').toLowerCase().includes(q)||(f.태그||[]).join(' ').toLowerCase().includes(q))
      groups['❓ FAQ'].push({title:f.질문||'',meta:f.대분류,action:()=>navigateTo('evaluation','eval-faq')});
  });
  SITE_DATA.resources.forEach(r=>{
    const title=r.제목||'';
    if(title.toLowerCase().includes(q)||(r.태그||[]).join(' ').toLowerCase().includes(q))
      groups['📂 자료'].push({title,meta:r.대분류,action:()=>navigateTo('archive')});
  });
...
function renderSearchDropdown(groups,query) {
  ...
  Object.entries(groups).forEach(([label,items])=>{
    ...
    items.slice(0,3).forEach((r,i)=>{
      html+=`<div class="search-result-item" data-group="${label}" data-idx="${i}">
  ...
  const allItems=[];
  Object.entries(groups).forEach(([label,items])=>
    items.slice(0,3).forEach(r=>allItems.push({label,r})));

  dd.querySelectorAll('.search-result-item').forEach((el,globalIdx)=>{
    el.addEventListener('click',()=>{
      allItems[globalIdx]?.r?.action?.();
